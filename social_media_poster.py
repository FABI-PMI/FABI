# social_media_poster.py (SOLO INSTAGRAM)
...
class SocialMediaPoster:
    ...
    def crear_imagen_ranking(self, usuarios_top):
        """Crea imagen optimizada para Instagram (1080x1080)"""
        ...
        try:
            ...
            font_pts = ImageFont.truetype("arial.ttf", 28)
        except:
        ...
        for i, (username, datos) in enumerate(usuarios_top[:5], 1):
            ...
            # Medalla
            draw.text((60, y_pos + 10), medal, fill='white', font=font_title)
            
            # Nombre
            draw.text((140, y_pos + 8), nombre, fill='white', font=font_name)
            
            # Puntos
            pts_text = f"{pts} pts"
            bbox = draw.textbbox((0, 0), pts_text, font=font_pts)
            pts_width = bbox[2] - bbox[0]
            draw.text((width - 120 - pts_width, y_pos + 22), pts_text, 
                     fill=medal_color, font=font_pts)
            
            y_pos += rect_height + 15
        
        # Fecha
        fecha = datetime.now().strftime('%d/%m/%Y')
        draw.text((width//2 - 60, height - 40), f"🎮 {fecha}", 
    ...
    def publicar_instagram(self, usuarios_top, caption_personalizado=None):
        ...
        try:
            ...
            if caption_personalizado:
                ...
                caption += "Top jugadores de Avatars VS Rooks:\n\n"
...
if __name__ == "__main__":
    print("🧪 MODO DE PRUEBA - INSTAGRAM\n")
